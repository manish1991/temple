        ...
        this.each(function () {
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    // Add ajax class in this.
                    $(createInput).find('.confirm').click(function () {
                        if ($(createInput).find('input').val() === '')
                            return;
                        var selected = getSelectedItems();
                        $('li').removeClass('newly-added');
                        var item = $('<li class="newly-added"><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');




                // AJAX function for saving question with options
                  
                  var label = $(createInput).find('input').val()
                  var question_group_id = $('#question_group_choice').val()
                  var question_option_type_id = $('#question_option_type_choice').val(),
                      parent_id = $('.li_selected').attr('nodeid');
                  ...
                  if(question_group_id){
                      $.ajax({
                                ...
                                type: 'post',
                                data: {'label': label, 'question_group_id': question_group_id,
                                       'csrfmiddlewaretoken':csrf_token},
                                dataType:'json',
                                cache: false,
                                success: function(response){
                                                question_id = response.question_id;
                                                $('.newly-added').attr('nodeid',response.question_id);
                                                $('#question_group_choice').val('');
                                                $('#question_option_type_choice').val('');
                                },
                    ...
                    else if(question_option_type_id){
                        $.ajax({
                                url: '/question/add-question-option/',
                                type: 'post',
                                data: {'option_label': label, 'question_option_type_id': question_option_type_id, 
                                       'question_id':question_id, 'parent_id':parent_id, 'csrfmiddlewaretoken':csrf_token},
                                dataType:'json',
                                cache: false,
                                success: function(response){
                                                $('.newly-added').attr('nodeid', response.question_option_id);
                                                $('#question_group_choice').val('');
                                                $('#question_option_type_choice').val('');
                                },
                                error: function(xhr, err){
